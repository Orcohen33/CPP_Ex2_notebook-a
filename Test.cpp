...
TEST_CASE("Test 1:Write and then read"){
    ...
    for(int i = 0 ; i<3000; i++){
        int rand_row = rand() % MAX_ROW;
        int rand_page = rand() % MAX_PAGE;
        int rand_col = rand() % 100;
        ...
        while((ulong)rand_col+rand_msg.size()> 100){
            rand_msg = randMessage();
        }
...
TEST_CASE("Test 2:Erase and read"){
    ...
    for(int i = 0 ; i<3000; i++){
        ...
        // if erase length is greater then 100 we regenerate new string
        while((ulong)(rand_col+len)> 100){
            len = rand() % 15;
        }
